    ...
    <div
      ...
      <div class="grid gap-2">
        <div class="grid grid-flow-col justify-between items-center">
          ...
          <PhoneLink :number="settings.phone"
            class="text-lg xl:text-xl text-blue-600 hover:text-blue-400 transition-colors hidden sm:block" />
        </div>
        <div class="text-lg lg:text-xl text-slate-600 font-thin hidden md:block" v-if="settings.tagline">{{ settings.tagline }}</div>
      </div>
      <nav class="hidden md:grid grid-flow-col gap-4 lg:gap-6 xl:gap-7 items-center justify-start font-thin text-slate-500">
        <template v-for="item of navItems">
          <NuxtLink v-if="item.type === 'built_in'" :to="item.built_in_page"
          ...
          <NuxtLink v-if="item.type === 'custom'" :to="'/page/' + item.custom_page"
            class="pr-4 lg:pr-6 xl:pr-7 last:pr-0 text-lg lg:text-xl hover:text-slate-800 transition-colors border-r-[1px] last:border-r-0"
            activeClass="text-slate-800 font-medium">
            {{ item.title }}
          </NuxtLink>
          <a v-if="item.type === 'external'" :href="item.external_link"
            class="pr-4 lg:pr-6 xl:pr-7 last:pr-0 text-lg lg:text-xl hover:text-slate-800 transition-colors border-r-[1px] last:border-r-0">
            {{ item.title }}
          </a>
        </template>
      ...
      <Sidebar v-model:visible="navOpen" position="full">
        <div class="grid gap-6 justify-center text-center">
          <template v-for="item of navItems">
            <NuxtLink v-if="item.type === 'built_in'" :to="item.built_in_page" @click="navOpen = false"
              class="sans-serif font-semibold text-slate-400 text-2xl hover:text-slate-800 transition-colors" activeClass="text-slate-900">
              {{ item.title }}
            </NuxtLink>
            <NuxtLink v-if="item.type === 'custom'" :to="'/page/' + item.custom_page" @click="navOpen = false"
              class="sans-serif font-semibold text-slate-400 text-2xl hover:text-slate-800 transition-colors" activeClass="text-slate-900">
              {{ item.title }}
            </NuxtLink>
            <a v-if="item.type === 'external'" :href="item.external_link"
              class="sans-serif font-semibold text-slate-400 text-2xl hover:text-slate-800 transition-colors">
              {{ item.title }}
            </a>
          </template>
        </div>
